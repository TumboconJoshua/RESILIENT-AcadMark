  ...
  <div class="w-full">
    <template v-if="!showBlank && !showGrade7Detail">
      ...
      <div v-else>
        <!-- ===================== Tab Navigation ===================== -->
        <div
          class="flex justify-between p-8 rounded-xl mb-6"
        ...
        >
          <div
            ...
            :class="activeTab === tab.value ? 'border-blue border-b-2' : ''"
            @click="setTab(tab.value)"
          >
            <p
              class="text-center pb-2 font-semibold text-lg"
        ...
        <div
          v-if="activeTab === 'add'"
          class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-[20px] my-8"
        >
          <div
        ...
        <div v-else class="submitted-section">
          ...
          <div
            class="bg-white p-6 border border-[#EEEEEE] rounded-[5px] shadow mb-6"
          >
            <!-- Filters -->
<div class="filters justify-between items-center flex-wrap mb-6">
  <!-- Filters Section -->
  <div class="flex flex-wrap gap-4 items-center">
    <select
      v-model="selectedSubmittedGrade"
    ...
    <select
      v-model="selectedSubmittedCurriculum"
      class="filter-dropdown"
    >
    ...
    <select
      v-model="selectedSubmittedTrack"
      class="filter-dropdown"
    >
    ...
    <select
      v-model="selectedSubmittedStatus"
      class="filter-dropdown"
    >
  ...
  <!-- Search Bar Section -->
  <div
    class="w-[350px] border border-[#295f98] flex items-center rounded-[8px] shadow-[rgba(0,0,0,0.05)_0px_6px_24px_0px,rgba(0,0,0,0.08)_0px_0px_0px_1px,rgba(0,0,0,0.1)_0px_0px_5px_0px,rgba(0,0,0,0.1)_0px_0px_1px_0px]"
  >
            ...
            <div class="rounded-lg border border-gray-200 overflow-x-auto mb-4">
              ...
              <div class="max-h-[400px] overflow-y-auto w-full">
                <table class="min-w-[900px] w-full border-collapse table-fixed">
                  <tbody>
                    <tr
                      v-for="(row, idx) in paginatedSuperClasses"
                      :key="idx"
    ...
    <div class="flex justify-center items-center pt-4 border-t border-gray-300 space-x-1">
            ...
            <span>Previous</span>
                ...
                <button
                  v-for="page in submittedTotalPages"
                  :key="page"
                  @click="submittedCurrentPage = page"
                  class="py-1 border border-[#295F98] rounded w-10 text-center"
                    :class="{
                        'bg-[#295F98] text-white': page === submittedCurrentPage,
                        'text-gray-600': page !== submittedCurrentPage,
                        'cursor-default': page === '...',
                        'cursor-pointer': page !== '...'
                    }"
    ...
    <template v-else-if="showGrade7Detail">
      <div class="min-h-[400px]">
        ...
        <div class="mt-6">
          <div
            class="flex gap-6 mb-6 bg-white p-[30px] border border-[#EEEEEE] rounded-xl shadow-[0_2px_8px_0_rgba(0,0,0,0.07)]"
          >
            ...
            <div class="min-w-[200px] relative flex flex-col justify-end">
              ...
              >
                ...
                <option
                  v-for="cls in filteredClassesByGrade"
                  :key="cls.Class_ID"
                  :value="cls"
                >
                  {{ cls.Section }}
            ...
            <div
              class="min-w-[200px] relative flex flex-col justify-end"
            ...
            >
              ...
              <select
                ...
                class="w-[180px] p-2 border border-[#E3E9EC] rounded-[6px] bg-white text-base text-[#242424] font-medium"
              >
                <option :value="selectedClass.Track">
                  {{ selectedClass.Track }}
                </option>
              </select>
            </div>
            <div class="flex items-end ml-auto">
              <button
                ...
                :class="[
                  ...
                  form.teacher_subject_ids.length === 0
                    ? 'bg-[#295F98] hover:bg-[#1d4066]'
          ...
          <div class="min-h-[200px] bg-[#f9f9f9] rounded-[8px] mt-8">
            <div class="flex gap-6 mb-5">
              ...
              <div
                class="flex-1 bg-white rounded-xl border border-[#e0e0e0] p-4 flex flex-col min-w-0"
              >
                <div class="flex flex-wrap items-center justify-between gap-3">
                  <!-- Gender Dropdown on the left -->
                  <div class="flex-shrink-0">
                    <select
                      v-model="rightGender"
                      class="filter-dropdown w-[130px] p-[6px_10px] border border-[#e0e0e0] rounded-[6px] text-[15px] text-[#242424] bg-white"
                    >
                  ...
                  <div class="flex items-center gap-2 ml-auto">
                    <div
                      class="w-[300px] border border-[#295f98] flex items-center rounded-[8px] shadow-[rgba(0,0,0,0.05)_0px_6px_24px_0px,rgba(0,0,0,0.08)_0px_0px_0px_1px,rgba(0,0,0,0.1)_0px_0px_5px_0px,rgba(0,0,0,0.1)_0px_0px_1px_0px]"
                    >
                      <i class="fas fa-search text-[#6C6C6C] text-lg mx-3"></i>
                      <input
                        type="text"
                    ...
                    <button
                      ...
                      class="bg-[#295F98] text-white rounded-[6px] px-[18px] py-2 text-[15px] disabled:opacity-50 cursor-pointer transition-colors duration-200 hover:bg-[#1d4066]"
                ...
                <div
                  class="flex-1 overflow-y-auto mb-[10px] max-h-[600px] relative"
                >
                  <table class="w-full border-collapse mt-4 bg-white">
                    <thead>
                      <tr>
                        <th
                          class="bg-[#F6F6F6] font-semibold p-2 text-center"
                        ></th>
                        <th class="bg-[#F6F6F6] font-semibold p-2 text-center">
                          LRN
                        </th>
                        <th class="bg-[#F6F6F6] font-semibold p-2 text-center">
                          FULL NAME
                      ...
                      <template v-else>
                        <tr>
                          <td
                            ...
                            class="text-center text-[#666] text-[15px] py-5 bg-white border-[#e0e0e0]"
                          >
                ...
                <div
                  class="flex justify-center items-center pt-4 border-t border-gray-300 space-x-1"
                >
                  <button
                  ...
                  <button
                    ...
                    class="py-1 border border-[#295F98] rounded w-10 text-center"
                    :class="{
                      'bg-[#295F98] text-white': page === studentCurrentPage,
                      'text-gray-600': page !== studentCurrentPage,
                      'cursor-default': page === '...',
                      'cursor-pointer': page !== '...',
                    }"
              ...
              >
                <div class="flex items-center justify-between mb-6">
                  <h1 class="text-[18px] font-semibold">Added Students</h1>
                  <button
                    :disabled="leftSelectedStudents.length === 0"
                    class="bg-[#295F98] text-white rounded-[6px] px-[18px] py-2 text-[15px] disabled:opacity-50 cursor-pointer transition-colors duration-200 hover:bg-[#1d4066]"
                    @click="removeAllSelectedStudents"
                  >
                    Remove Students
                  </button>
                </div>
                ...
                >
                  <template v-if="leftSelectedStudents.length > 0">
                    <div
                      ...
                      :key="row.lrn"
                      class="flex items-center p-[6px_8px] border-[#f0f0f0] text-sm text-[#222] gap-10 hover:bg-[#e0e0e0]"
                    >
                      <span class="ml-[5px] w-[90px] font-medium">{{
                        row.lrn
                      }}</span>
                      <span class="flex-1 -ml-[5px] font-medium">{{
                        row.name
                      }}</span>
                      <img
                        ...
                        @click="removeSelectedRow(row.lrn)"
                      />
                    </div>
                  </template>
                  <template v-else>
    ...
    <template v-else>
      <div class="flex items-start">
        <button @click="showBlank = false" class="flex items-center mt-2 ml-2">
          ...
          >
          <span
            class="ml-1 text-lg font-semibold text-[#242424] underline cursor-pointer"
            >Back</span
          >
        </button>
      </div>
  ...
  >
    ...
    >
      <h2 class="text-[1.5rem] font-bold text-[#295F98] mb-7">
        Add Class Teacher
      </h2>
      <div class="mb-[22px]">
        <div class="text-[#295F98] text-[1rem] font-semibold mb-2">
          Advisory Teacher
        </div>
        <div class="flex mb-2 justify-between">
          <select
            v-model="selectedSubject"
            @change="logSelectedAdviserSubjectId"
          ...
          >
            ...
            <option
              v-for="adviser in filteredAdvisersBySubject"
              :key="adviser.id"
              :value="adviser.id"
            >
              {{ adviser.name }}
          ...
          <div class="flex flex-col relative">
            ...
            <input
              ...
              :value="selectedAdviserEmployeeNo"
              readonly
            />
          </div>
        </div>
      ...
      <div class="mb-[22px]">
        ...
        <div
          v-for="(teacherEntry, index) in subjectTeachers"
          :key="index"
          class="flex gap-[22px] mb-2 items-start justify-between"
        >
          <select
            v-model="teacherEntry.subject"
            @change="logTeacherSubjectId"
            class="min-w-[220px] h-[48px] px-[12px] mb-2 border border-[#E3E9EC] rounded-[6px] bg-white text-[1rem] text-[#242424] font-medium"
          >
            <option disabled value="">Select Subject</option>
            <option
          ...
          >
            ...
            <option
              v-for="teacher in filteredTeachersBySubject(teacherEntry.subject)"
              :key="teacher.id"
              :value="teacher.name"
            >
              {{ teacher.name }}
          ...
          <div class="flex flex-col relative">
            ...
            <input
              ...
              placeholder="Auto Fill"
              :value="adviserIdByName(teacherEntry.teacher)"
              readonly
            />
          </div>
          ...
          <button
            @click="removeSubjectTeacher(index)"
  ...
  >
    <div class="bg-white w-[750px] rounded-lg p-8 shadow-lg flex flex-col">
      ...
      <div class="grid grid-cols-2 gap-x-8 gap-y-2 mb-6">
        <div>
          <div class="text-[#295F98] text-[13px] font-semibold mb-1">
            Class Adviser
          </div>
      ...
      <textarea
        :value="selectedClassInfo.comments"
        readonly
        class="w-full h-[120px] border border-[#295F98] rounded-lg p-3 text-[15px] mb-6 resize-none"
      ></textarea>
      <div class="flex justify-end">
        <button
          @click="closeClassInfoModal"
          class="bg-[#888] text-white px-8 py-2 rounded transition hover:bg-[#444] cursor-pointer"
        >
          Close
        </button>
      </div>
...
<script>
  ...
  import {
    ...
    getClassesExcludingIncomplete,
  } from '@/service/teacherSubjectsService';
  ...
  export default {
    ...
    watch: {
      ...
      selectedRows(newLRNs) {
        ...
        this.form.student_ids = this.leftSelectedStudents.map(
          (row) => row.student_id
        );
      },
    ...
    methods: {
      ...
      logTeacherSubjectId() {
        ...
        const subject = this.subjectOptions.find(
          (s) => s.name === this.teacherEntry.subject
        );
        console.log(
          'Selected Teacher Subject Name:',
          this.teacherEntry.subject
        );
        console.log(
          'Selected Teacher Subject ID:',
      ...
      adviserIdByName(name) {
        ...
        const adviser = this.teacherOptions.find(
          (adviser) => adviser.name === name
        );
        console.log(
          'adviserIdByName:',
          name,
          '=>',
          adviser ? adviser.id : 'NOT FOUND'
        );
        return adviser ? adviser.id : '';
      },

      subjectIdByName(name) {
        const match = this.subjectOptions.find(
      ...
      submitFacultyAssignments() {
        ...
        const adviserSubject = this.subjectOptions.find(
          (s) => s.name === this.selectedSubject
        );
        const adviserSubjectId = adviserSubject ? adviserSubject.id : '';

        if (adviserId && adviserSubjectId) {
          const adviserTeacherSubjectId = this.getTeacherSubjectId(
            adviserId,
            adviserSubjectId
          );
        ...
        for (const entry of this.subjectTeachers) {
          ...
          if (teacherId && subjectId) {
            const teacherSubjectId = this.getTeacherSubjectId(
              teacherId,
              subjectId
            );
            if (teacherSubjectId) {
      ...
      handleDrop() {
        if (
          this.draggedStudent &&
          !this.leftSelectedStudents.some(
            (s) => s.student_id === this.draggedStudent.student_id
          )
        ) {
      ...
      removeSelectedRow(lrn) {
        const removed = this.leftSelectedStudents.find((s) => s.lrn === lrn);

        // Remove from right container
        this.leftSelectedStudents = this.leftSelectedStudents.filter(
          (s) => s.lrn !== lrn
        );

        // ✅ Return to left table
        if (removed) {
          this.students.push(removed);
        }
      ...
      async fetchStudentCounts() {
        try {
          ...
          this.dummyTableData = this.grades.map(
            (grade) => tableDataByGrade[grade.mainTitle] || []
          );
        } catch (error) {
    ...
    computed: {
      filteredTableData() {
        ...
        // ✅ Exclude students already in right container
        const selectedIds = this.leftSelectedStudents.map((s) => s.student_id);
        filteredData = filteredData.filter(
          (row) => !selectedIds.includes(row.student_id)
        );

        // 🔹 Filter by Track
        if (this.selectedClass?.Track) {
          filteredData = filteredData.filter(
            (row) => row.track === this.selectedClass.Track
          );
        }
      ...
      filteredLeftTableData() {
        if (!this.leftSex || this.leftSex === 'All')
          return this.leftSelectedStudents;
      ...
      filteredAdvisersBySubject() {
        ...
        const subjectObj = this.subjectOptions.find(
          (s) => s.name === this.selectedSubject
        );
      ...
      teacherButtonLabel() {
        return this.form.teacher_subject_ids.length === 0
          ? 'Add Teacher'
          : 'Edit Teacher';
      },
      gradeOptions() {
    const grades = this.superClasses.map(row => row.grade);
    return [...new Set(grades)].sort(); // Remove duplicates and sort
  },
  curriculumOptions() {
    const curricula = this.superClasses.map(row => row.curriculum);
    return [...new Set(curricula)].sort();
  },
  trackOptions() {
    const tracks = this.superClasses.map(row => row.track);
    return [...new Set(tracks)].sort();
  },
  statusOptions() {
    const statuses = this.superClasses.map(row => row.status);
    return [...new Set(statuses)].sort();
  },

  filteredSuperClasses() {
    return this.superClasses.filter((row) => {
      const matchesGrade = !this.selectedSubmittedGrade || row.grade === this.selectedSubmittedGrade;
      const matchesCurriculum = !this.selectedSubmittedCurriculum || row.curriculum === this.selectedSubmittedCurriculum;
      const matchesTrack = !this.selectedSubmittedTrack || row.track === this.selectedSubmittedTrack;
      const matchesStatus = !this.selectedSubmittedStatus || row.status === this.selectedSubmittedStatus;

      const rowText = Object.values(row).join(' ').toLowerCase();
      const matchesSearch = !this.searchQuery || rowText.includes(this.searchQuery.toLowerCase());
  ...
  paginatedSuperClasses() {
    ...
    return this.filteredSuperClasses.slice(start, start + this.itemsPerPage);
  },

  submittedTotalPages() {
    return Math.ceil(this.filteredSuperClasses.length / this.itemsPerPage) || 1;
  },
