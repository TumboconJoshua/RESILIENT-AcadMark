<template>
  <div class="w-full">
    ...
    <div
      class="flex justify-between p-8 rounded-xl mb-8"
    ...
    <div class="my-5">
      ...
      <div class="flex flex-col gap-10" v-if="activeTab === 'add'">
        <div
          class="p-7 rounded-xl"
          style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px"
        >
          <p class="font-semibold text-xl text-blue mb-5">
            Individual Registration Form
          </p>

          <form
            @submit.prevent="handleAddStudentSubmit"
          ...
          >
            ...
            >
              <div
                v-for="addStudentField in row"
                :key="addStudentField.name"
                :class="addStudentField.class || 'relative grow'"
              >
                <label
                  class="absolute top-[-10px] left-[20px] text-sm bg-white px-2"
                >
                ...
                <select
                  v-if="addStudentField.type === 'select'"
                ...
                >
                  <option disabled value="">
                    {{ addStudentField.placeholder }}
                  </option>
                  <option
                    v-for="option in addStudentField.options"
                    :key="option"
                    :value="option"
                  >
                    {{ option }}
                ...
                <input
                  v-else
                  :type="addStudentField.type"
                  v-model="formData[addStudentField.name]"
                  class="w-full py-3 px-5 border rounded"
                  :placeholder="addStudentField.placeholder"
                  :maxlength="addStudentField.maxLength"
                  :pattern="addStudentField.pattern"
                  :title="addStudentField.title"
                  :required="addStudentField.required"
                />
          ...
          <form @submit.prevent="handleBulkSubmit" class="flex flex-col gap-5">
            ...
            <div class="flex flex-row gap-5 w-[70%]">
              <div class="relative grow">
                ...
                >
                <select
                  v-model="bulkFormData.gradeLevel"
                  class="w-full py-3 px-5 border rounded appearance-none"
                  required
                >
                  <option value="">Select Grade Level</option>
                  <option
                    v-for="level in bulkFormFields.gradeLevel.options"
                    :key="level"
                    :value="level"
                  >
                    {{ level }}
              ...
              <div class="relative grow">
                ...
                >
                <select
                  v-model="bulkFormData.curriculum"
                  class="w-full py-3 px-5 border rounded appearance-none"
                  required
                >
                  <option disabled value="">Select Curriculum</option>
                  <option
                    v-for="level in bulkFormFields.curriculum.options"
                    :key="level"
                    :value="level"
                  >
                    {{ level }}
              ...
              <div class="relative grow">
                ...
                >
                <select
                  v-model="bulkFormData.track"
                  class="w-full py-3 px-5 border rounded appearance-none"
                  required
                >
                  <option disabled value="">Select Track</option>
                  <option
                    v-for="level in bulkFormFields.track.options"
                    :key="level"
                    :value="level"
                  >
                    {{ level }}
            ...
            <div class="mt-5">
              <div
                class="gap-4 justify-center w-full border border-black rounded-lg"
              >
          ...
          >
            <div class="flex justify-between mb-6">
              <div class="flex flex-wrap gap-5">
                ...
                <Dropdown
                  :showAcademicTrack="true"
                  v-model="selectedAcademicTrack"
                />
              </div>
              <div class="w-[30%]">
            ...
            <div class="overflow-auto">
              <table class="w-full border-collapse text-center text-sm">
                <thead
                  class="bg-gray-100 text-[#464F60] text-[15px] font-semibold"
                >
                  <tr>
                ...
                <tbody class="font-medium text-[15px]">
                  <tr v-if="filteredStudents.length === 0">
                    <td colspan="9" class="p-2 text-center">
            ...
            <div
              class="flex justify-center items-center mt-auto pt-4 border-t border-gray-300 space-x-1"
            >
              <button
    ...
    <div
      v-if="showModal"
      class="fixed inset-0 z-50 bg-black/50 flex justify-center items-center p-[5%]"
    >
      <div
        class="bg-white p-10 flex flex-col gap-5 max-h-[90vh] overflow-y-auto"
      >
        <div class="flex justify-between items-center">
          <p class="font-semibold text-2xl text-blue">
            Individual Registration Form
          </p>
        </div>
        ...
        <form @submit.prevent="handleUpdateStudent" class="flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-2">
            <label class="text-blue font-semibold text-2xl" for="comment"
              >COMMENT</label
            >
            <textarea
              v-model="comment"
              rows="10"
              class="border-1 rounded-lg p-2"
              placeholder="Superadmin comment here..."
              disabled
            ></textarea>
          </div>
          ...
          <div class="flex justify-end gap-2 mt-5">
            ...
            <template v-if="isEditing">
              <button
                type="button"
                @click="openDropModal(selectedStudent?.original)"
    ...
    <div
      v-if="isModalDropOpen"
      class="fixed inset-0 z-50 bg-black/50 flex justify-center items-center p-[5%]"
    >
      <div
        class="bg-white p-10 gap-5 max-h-[90vh] w-[700px] overflow-y-auto rounded-lg"
      >
        <div class="flex justify-between items-center">
        ...
        <form class="flex flex-col gap-5">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="floating-label mb-4">
              <input
                ...
                class="input"
                placeholder=" "
                :value="dropOutFormData.Grade_Level"
                readonly
              />
            ...
            <div class="floating-label mb-4">
              <input
                ...
                class="input"
                placeholder=" "
                :value="dropOutFormData.Curriculum"
                readonly
              />
          ...
          <div class="floating-label mb-4">
            <input
              ...
              class="input"
              placeholder=" "
              :value="dropOutFormData.FullName"
              readonly
            />
...
<script setup>
  ...
  const filteredList = computed(() => {
    ...
    if (selectedGrade.value) {
      results = results.filter(
        (student) => student.gradeLevel === selectedGrade.value
      );
    }
    if (selectedCurriculum.value) {
      results = results.filter(
        (student) => student.curriculum === selectedCurriculum.value
      );
    }
    if (selectedSex.value) {
      results = results.filter((student) => student.sex === selectedSex.value);
    }
    if (selectedAcademicTrack.value) {
      results = results.filter(
        (student) => student.track === selectedAcademicTrack.value
      );
    }
  ...
  function openDropModal(student) {
    ...
    dropOutFormData.Grade_Level = student.Grade_Level
      ? `Grade ${student.Grade_Level}`
      : '';
    dropOutFormData.Curriculum =
  ...
  const addStudentFields = [
    ...
    },
    {
      ...
      maxLength: 2,
      pattern: '\\d{1,2}',
      title: 'Age must be a two-digit number',
      required: true,
      row: 2,
      class: 'relative grow max-w-[100px]',
    },
  ...
  const formData = reactive(
    Object.fromEntries(addStudentFields.map((f) => [f.name, '']))
  );
  ...
  watch(
    ...
    (newValue) => {
      ...
      const curriculumField = addStudentFields.find(
        (field) => field.name === 'Curriculum'
      );
      if (curriculumField) {
      ...
      const trackField = addStudentFields.find(
        (field) => field.name === 'Track'
      );
      if (trackField) {
  ...
  async function handleAddStudentSubmit() {
    try {
      ...
      if (dataToSend.Curriculum === 'Junior High School')
        dataToSend.Curriculum = 'JHS';
      else if (dataToSend.Curriculum === 'Senior High School')
        dataToSend.Curriculum = 'SHS';
  ...
  async function handleUpdateStudent() {
    ...
    try {
      ...
      if (updatedData.Curriculum === 'Junior High School')
        updatedData.Curriculum = 'JHS';
      else if (updatedData.Curriculum === 'Senior High School')
        updatedData.Curriculum = 'SHS';
  ...
  watch(
    ...
    (newValue) => {
      ...
      bulkFormFields.curriculum.options = [
        'Junior High School',
        'Senior High School',
      ];

      // Update track options based on curriculum for bulk form
      bulkFormFields.track.options =
        bulkTrackOptions[bulkFormData.value.curriculum] || [];
    }
  );
